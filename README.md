...
`iomon web` 提供实时视图和告警页 `/alerts`。告警按进程记：某一秒 IO 占比达到阈值（默认 20%）就会留下记录，不必持续。历史写在 `deploy/alerts`，保留 7 天，更早的自动删掉。页面上可以改阈值、按天查看。Docker Compose 默认就是这个命令，映射宿主机 8080 端口。
